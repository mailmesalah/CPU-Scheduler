...
int main(int argc, char *argv[])
{
	...
	if(!roundRobin)
	{
		...
		/************************************************************************/
			...
			/*for(int i=0;i<numberOfProcesses;++i)
			{
				for(int j=0;j<processes[i].numberOfThreads;++j)
				{
					...
					{
						if(k!=processes[i].threads[j].numberOfCPU-1)
						{
							cpuBurstTime=cpuBurstTime+processes[i].threads[j].cPUIOTimes[k].cPUTime;	
							totalTime=totalTime+processes[i].threads[j].cPUIOTimes[k].cPUTime;	
							ioBurstTime=ioBurstTime+processes[i].threads[j].cPUIOTimes[k].iOTime;	
							totalTime=totalTime+processes[i].threads[j].cPUIOTimes[k].iOTime;	
							//cout<<cpuBurstTime<<"+"<<ioBurstTime<<"="<<totalTime<<"\n";
						}
						else
						{
							cpuBurstTime=cpuBurstTime+processes[i].threads[j].cPUIOTimes[k].cPUTime;	
				...
				//cout<<processes[i].numberOfThreads;
		...
		/************************************************************************/
			...
			cout<<"FCFS:\n\n";
			cout<<"Total Time required is "<<totalTime<<" time units\n";
			cout<<"Average Turnaround Time is "<<(float)totalTurnAroundTime/numberOfProcesses<<" time units\n";
			//cout<<"CPU Utilization is "<<(float)(cpuBurstTime*100)/totalTime<<" cpu burst" << cpuBurstTime<<" IO Burst Time "<<ioBurstTime<<"\n";;
